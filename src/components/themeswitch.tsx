...
>(({ className, ...props }, ref) => (
  ...
  >
    <SwitchPrimitives.Thumb
      className={cn(
        'pointer-events-none flex h-7 w-7 items-center justify-center rounded-full bg-background shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-8 data-[state=unchecked]:translate-x-0',
      )}
    >
      {props.checked ? <ZapIcon className="h-4 w-4" /> : <SunIcon className="h-4 w-4" />}
    </SwitchPrimitives.Thumb>
  </SwitchPrimitives.Root>
))
